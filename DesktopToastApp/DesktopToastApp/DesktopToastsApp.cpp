...
HRESULT DesktopToastsApp::CreateToastXml(_In_ IToastNotificationManagerStatics *toastManager, _Outptr_ IXmlDocument** inputXml)
{
	...
	if (SUCCEEDED(hr))
	{
		wchar_t *imagePath = _wfullpath(nullptr, L"shield.png", MAX_PATH);

		hr = imagePath != nullptr ? S_OK : HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND);
		if (SUCCEEDED(hr))
		{
			hr = SetImageSrc(imagePath, *inputXml);
			PCWSTR textValues[] = { L"Vertex Security is great!"};
			UINT32 textLengths[] = { 25 };
...
HRESULT DesktopToastsApp::SetTextValues(const PCWSTR* textValues, _In_ UINT32 textValuesCount, _In_reads_(textValuesCount) UINT32 *textValuesLengths, _In_ IXmlDocument *toastXml)
{
